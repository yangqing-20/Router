...
RoutingTable::lookup(uint32_t ip) const
{
  ...
  //  RoutingTableEntry has 4 fields: dest, gw, mask, ifName
  // gw for gateway ip address
  // ifName for the interface name
  int match_pre_len = -1;
  RoutingTableEntry res;
  for (auto e : m_entries) {
      if ((ip & e.mask) == (e.dest & e.mask)) {//the ip matches the dest
          int len = 0;
          uint32_t mask = e.mask;
          while (mask) { // count the number of 1s in the mask
            len += mask & 1;
            mask >>= 1;
          }
          if (match_pre_len < len) {
              match_pre_len = len;
              res = e;
          }
      }
